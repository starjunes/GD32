...
static void HdlGprsFrame(INT8U *sptr, INT32U slen)
{
    ...
    if (!YX_PROTOCOL_HandleFrame(&attrib, sptr, slen)) {
        #if DEBUG_TLINK > 0
        printf_com("<yx_jt1_trecv, sorry, protocol check error>\r\n");
        #endif
    }
}
...
static void RecvTmrProc(void *pdata)
{
    ...
    for (;;) {
        ...
        if (s_rcb.rlen == 0) {
            if (ch == g_jtt1_rules.c_flags) {                                    /* 检测协议头 */
                s_rcb.rbuf[0] = 0;
                s_rcb.rlen++;
            }
        } else {
            if (ch == g_jtt1_rules.c_flags) {
                if (s_rcb.rlen > 1) {
                    s_rcb.rlen = YX_DeassembleByRules(s_rcb.rbuf, &s_rcb.rbuf[1], s_rcb.rlen - 1, (ASMRULE_T *)&g_jtt1_rules);
                    if (s_rcb.rlen >= 2) {
...
/*******************************************************************
** 函数名:     YX_JTT1_InitRecv
** 函数描述:   初始化接收模块
** 参数:       无
** 返回:       无
********************************************************************/
void YX_JTT1_InitRecv(void)
{
    s_rcb.rlen = 0;
...
/*******************************************************************
** 函数名:     YX_JTT1_RecvData
** 函数描述:   接收解析数据
** 参数:       [in] socketid: socket编号 
** 返回:       无
********************************************************************/
void YX_JTT1_RecvData(INT8U socketid, INT8U *sptr, INT32U slen)
{
    INT8U ch;
    INT32U i;
    ...
    for (i = 0; i < slen; i++) {
        ...
        if (s_rcb.rlen == 0) {
            ...
            if (ch == g_jtt1_rules.c_flags) {
                if (s_rcb.rlen > 1) {
                    s_rcb.rlen = YX_DeassembleByRules(s_rcb.rbuf, sizeof(s_rcb.rbuf), &s_rcb.rbuf[1], s_rcb.rlen - 1, &g_jtt1_rules);
                    if (s_rcb.rlen >= 2) {
                        HdlGprsFrame(s_rcb.rbuf, s_rcb.rlen);
                    }
                    s_rcb.rlen = 0;
