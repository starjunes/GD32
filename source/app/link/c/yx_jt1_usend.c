...
/*******************************************************************
** 函数名:     YX_JTU1_SendHeart
** 函数描述:   发送链路维护帧
** 参数:       无
** 返回:       成功返回true，失败返回false
********************************************************************/
BOOLEAN YX_JTU1_SendHeart(void)
{
    INT32U msgattrib;
    TEL_T alarmtel;
    STREAM_T *wstrm;
...
/*******************************************************************
** 函数名:     YX_JTU1_SendReqRegist
** 函数描述:   发送注册请求数据包
** 参数:       无
** 返回:       成功返回true，失败返回false
********************************************************************/
BOOLEAN YX_JTU1_SendReqRegist(void)
{
...
/*******************************************************************
** 函数名:     YX_JTU1_SendReqUnregist
** 函数描述:   发送注销请求数据包
** 参数:       无
** 返回:       成功返回true，失败返回false
********************************************************************/
BOOLEAN YX_JTU1_SendReqUnregist(void)
{
    INT32U msgattrib;
    TEL_T alarmtel;
    STREAM_T *wstrm;
...
/*******************************************************************
** 函数名:     YX_JTU1_SendReqLogin
** 函数描述:   发送登陆请求数据包
** 参数:       无
** 返回:       成功返回true，失败返回false
********************************************************************/
BOOLEAN YX_JTU1_SendReqLogin(void)
{
...
** 返回:       成功返回true,失败返回false
********************************************************************/
BOOLEAN YX_JTU1_SendFromGPRS(PROTOCOL_COM_T *attrib, INT8U *data, INT16U datalen)
{
    ...
    if (attrib->type == PTOTOCOL_TYPE_LOG) {                                   /* 登陆帧 */
        if (!YX_JTU1_LinkIsConnect()) {                                        /* UDP链路未建立 */
            return false;
        }
    } else {                                                                   /* 普通数据 */
        if (!YX_JTU1_LinkIsLoged()) {                                          /* 未登陆到GPRS前置机 */
            return false;
        }
    }
    ...
    if (memptr == 0) {
        #if DEBUG_ERR > 0
        printf_com("<YX_JTU1_SendFromGPRS malloc memory fail>\r\n");
        #endif
