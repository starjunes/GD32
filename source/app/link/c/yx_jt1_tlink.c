...
static void Turninto_Connecting(void)
{
    ...
    if (AT_SOCKET_SocketIsClosed(COM_TCP)) {                                   /* 关闭状态 */
        if (++s_lcb.ct_connect <= MAX_CONNECT) {
            ...
            #if DEBUG_TLINK > 0
            printf_com("<第一服务器TCP连接请求超过次数>\r\n");
            #endif
            
            s_lcb.ct_connect = 0;
            s_lcb.curindex++;
            CheckGprsPara();
            if (!YX_JTT2_LinkCanCom()) {                                       /* 第二服务器通信也异常 */
...
static void Turninto_Reging(void)
{
    ...
    if (++s_lcb.ct_reg <= MAX_REG) {
        #if DEBUG_TLINK > 0
        printf_com("<第一服务器TCP注册请求(%d)>\r\n", s_lcb.ct_reg);
        #endif
        
        OS_StartTmr(s_monitortmr, PERIOD_REG);
        
        AT_SOCKET_ClearDataBySocket(COM_TCP);
        YX_JTT1_SendReqRegist();
    } else {
        #if DEBUG_TLINK > 0
        printf_com("<第一服务器TCP注册请求超过次数>\r\n");
        #endif
        
        s_lcb.ct_reg = 0;
		YX_JTT1_LinkReconnect(DELAY_REG);
    }
}
...
static void Turninto_Loging(void)
{
    if (++s_lcb.ct_log <= MAX_LOG) {
        #if DEBUG_TLINK > 0
        printf_com("<第一服务器TCP登入请求(%d)>\r\n", s_lcb.ct_log);
        #endif
        
        OS_StartTmr(s_monitortmr, PERIOD_LOG);
        
        AT_SOCKET_ClearDataBySocket(COM_TCP);
        YX_JTT1_SendReqLogin();                                                /* 发送登入帧 */
    } else {
        #if DEBUG_TLINK > 0
        printf_com("<第一服务器TCP登入请求超过次数>\r\n");
        #endif
        
        s_lcb.ct_log = 0;
        YX_JTT1_LinkReconnect(0);                                              /* 重新连接 */
    }
}
...
{
    if (YX_JTT1_LinkIsOpen()) {                                                /* 处于激活状态 */
        OS_ASSERT(OS_TmrIsRun(s_monitortmr), RETURN_VOID);
    }
}

/*******************************************************************
** 函数名:     YX_JTT1_InitLink
** 函数描述:   连接模块初始化
** 参数:       无
** 返回:       无
********************************************************************/
...
/*******************************************************************
** 函数名:     YX_JTT1_LinkIsOpen
** 函数描述:   链路流程是否已开启
** 参数:       无
** 返回:       是返回true，否返回false
********************************************************************/
BOOLEAN YX_JTT1_LinkIsOpen(void)
{
...
/*******************************************************************
** 函数名:     YX_JTT1_OpenLink
** 函数描述:   申请应用
** 参数:       无
** 返回:       已登入返回true，为登入返回false
********************************************************************/
BOOLEAN YX_JTT1_OpenLink(void)
{
    if ((s_lcb.status & APPLY_) == 0) {
        s_lcb.status = APPLY_;
    ...
    return YX_JTT1_LinkIsLoged();
}

/*******************************************************************
** 函数名:     YX_JTT1_CloseLink
** 函数描述:   释放应用
** 参数:       无
** 返回:       已登入返回true，为登入返回false
********************************************************************/
BOOLEAN YX_JTT1_CloseLink(void)
{
    INT8U result;
    
    result = YX_JTT1_LinkIsLoged();
    if ((s_lcb.status & APPLY_) != 0) {
        s_lcb.status = 0;
...
/*******************************************************************
** 函数名:     YX_JTT1_LinkInformGprsActived
** 函数描述:   GPRS已激活/已去活回调
** 参数:       [in]  result:  true－表示激活成功，flase表示去活回调
** 返回:       无
********************************************************************/
void YX_JTT1_LinkInformGprsActived(INT8U result)
{
    #if DEBUG_TLINK > 0
    printf_com("<YX_JTT1_LinkInformGprsActived(%d)>\r\n", result);
    #endif
...
/*******************************************************************
** 函数名:     YX_JTT1_LinkInformConnect
** 函数描述:   socket已连接/断开回调处理
** 参数:       [in]  result:  true－表示已连接，flase表示已断开
** 返回:       无
********************************************************************/
void YX_JTT1_LinkInformConnect(INT8U result)
{
    #if DEBUG_TLINK > 0
    printf_com("<YX_JTT1_LinkInformConnect(%d)>\r\n", result);
    #endif
...
** 返回:       无
********************************************************************/
void YX_JTT1_LinkInformReged(INT8U result, INT8U *sptr, INT8U slen)
{
    AUTH_CODE_T authinfo;
    
    #if DEBUG_TLINK > 0
    printf_com("<YX_JTT1_LinkInformReged(%d)(%d)>\r\n", result, slen);
    #endif
...
/*******************************************************************
** 函数名:     YX_JTT1_LinkInformLoged
** 函数描述:   登入结果通知
** 参数:       [in]  result:  true－成功，flase-失败
** 返回:       无
********************************************************************/
void YX_JTT1_LinkInformLoged(INT8U result)
{
    #if DEBUG_TLINK > 0
    printf_com("<YX_JTT1_LinkInformLoged(%d)>\r\n", result);
    #endif
...
/*******************************************************************
** 函数名:     YX_JTT1_LinkInformQuery
** 函数描述:   链路维护通知
** 参数:       [in] index:传递参数,根据具体参数
** 返回:       无
********************************************************************/
void YX_JTT1_LinkInformQuery(void *index)
{
    if (YX_JTT1_LinkIsLoged()) {
        #if DEBUG_TLINK > 0
        printf_com("<第一服务器TCP心跳维护(%d)>\r\n", (INT32U)index);
        #endif
...
/*******************************************************************
** 函数名:     YX_JTT1_LinkReconnect
** 函数描述:   链路socket断开后再重新连接
** 参数:       [in] delay: 重新连接等待时间，单位秒，0－立即断开重连
** 返回:       无
********************************************************************/
void YX_JTT1_LinkReconnect(INT32U delay)
{
    #if DEBUG_TLINK > 0
    printf_com("<YX_JTT1_LinkReconnect (%d) status:0x%x>\r\n", delay, s_lcb.status);
    #endif
...
/*******************************************************************
** 函数名:     YX_JTT1_LinkIsLoged
** 函数描述:   是否已登入
** 参数:       无
** 返回:       是返回true，否返回false
********************************************************************/
BOOLEAN YX_JTT1_LinkIsLoged(void)
{
...
/*******************************************************************
** 函数名:     YX_JTT1_LinkIsReged
** 函数描述:   链路是否已注册
** 参数:       无
** 返回:       是返回true，否返回false
********************************************************************/
BOOLEAN YX_JTT1_LinkIsReged(void)
{
...
/*******************************************************************
** 函数名:     YX_JTT1_LinkIsConnect
** 函数描述:   链路socket是否已连接
** 参数:       无
** 返回:       是返回true，否返回false
********************************************************************/
BOOLEAN YX_JTT1_LinkIsConnect(void)
{
...
/*******************************************************************
** 函数名:     YX_JTT1_LinkCanCom
** 函数描述:   通道是否可以通信
** 参数:       无
** 返回:       是返回true，否返回false
********************************************************************/
BOOLEAN YX_JTT1_LinkCanCom(void)
{
    if (!YX_JTT1_LinkIsLoged()) {
        return FALSE;
    } else {
