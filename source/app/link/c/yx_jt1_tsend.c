...
BOOLEAN YX_JTT1_SendFromGPRS(PROTOCOL_COM_T *attrib, INT8U *data, INT16U datalen)
{
    ...
    if (attrib->type == PTOTOCOL_TYPE_LOG) {                                   /* 登陆帧 */
        if (!YX_JTT1_LinkIsConnect()) {                                        /* TCP链路未建立 */
            return false;
        }
    } else {                                                                   /* 普通数据 */
        if (!YX_JTT1_LinkIsLoged()) {                                          /* 未登陆到GPRS前置机 */
            return false;
        }
    }
    ...
    if (memptr == 0) {
        #if DEBUG_ERR > 0
        printf_com("<YX_JTT1_SendFromGPRS malloc memory fail>\r\n");
        #endif
        
        return false;
    }
    
    templen = YX_AssembleByRules(memptr, memlen, data, datalen, (ASMRULE_T *)&g_jtt1_rules);
