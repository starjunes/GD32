...
** 文件描述:   该模块主要实现交通部协议第一服务器UDP注册、登陆管理
** 
*********************************************************************************
**             修改历史记录
**===============================================================================
...
********************************************************************************
*/
#define COM_UDP                     CHA_JT1_ULINK          /* GPRS通信通道 */
...
static BOOLEAN CheckAuthCode(void)
{
    ...
    DAL_PP_ReadParaByID(PP_ID_AUTHCODE1, (INT8U *)&authinfo, sizeof(authinfo));
...
static BOOLEAN SendHeartData(void)
{
    ...
    if (++s_lcb.ct_heart <= s_lcb.heart_max) {                                 /* 链路维护请求超过次数 */
        ...
        #if DEBUG_ULINK > 0
        printf_com("<第一服务器心跳维护请求超过次数,周期(%d)秒,等待(%d),次数(%d)>\r\n", s_lcb.heart_period, s_lcb.heart_waittime, s_lcb.heart_max);
        #endif
        
        s_lcb.ct_heart = 0;
		YX_JTU1_LinkReactivate(0);
    }
}
...
static BOOLEAN CheckGprsPara(void)
{
    ...
    if (!DAL_PP_ReadParaByID(PP_ID_GPRSPARA1, (INT8U *)&s_gprspara, sizeof(s_gprspara))) {
        return false;
    }
...
static void Turninto_LOGED(void)
{
    if ((s_lcb.status & (ACTIVE_ | MASK_)) == (ACTIVE_ | CONNECT_ | REGED_)) { /* 已注册成功 */
        ...
        YX_StartTmr(s_monitortmr, SECOND, 1, LOW_PRECISION_TIMER);
        OS_PostMsg(PRIO_OPTTASK, MSG_UDPLINK_LOGED, COM_UDP, 0);
    }
}
...
static void Turninto_REGED(void)
{
    if ((s_lcb.status & (ACTIVE_ | MASK_)) == (ACTIVE_ | CONNECT_)) {
        #if DEBUG_ULINK > 0
        printf_com("<第一服务器UDP注册成功>\r\n");
        #endif
        
        s_lcb.status |= REGED_;
        YX_StartTmr(s_monitortmr, SECOND, 1, LOW_PRECISION_TIMER);             /* 延时几秒开始登入请求 */
        OS_PostMsg(PRIO_OPTTASK, MSG_UDPLINK_REGED, COM_UDP, 0);
    }
}
...
{
    if (YX_JTU1_LinkIsActived()) {                                             /* 应用已启动的条件下 */
        #if DEBUG_ULINK > 0
        printf_com("<第一服务器UDP登出>\r\n");
        #endif
...
static void Turninto_Reging(void)
{
    ...
    if (++s_lcb.ct_reg <= MAX_REG) {
        #if DEBUG_ULINK > 0
        printf_com("<第一服务器UDP注册请求(%d)>\r\n", s_lcb.ct_reg);
        #endif
        
        YX_StartTmr(s_monitortmr, PERIOD_REG);
        
        DAL_UDP_ClearBuf(COM_UDP);
        YX_JTU1_SendReqRegist();
    } else {
        #if DEBUG_ULINK > 0
        printf_com("<第一服务器UDP注册请求超过次数>\r\n");
        #endif
        
        s_lcb.ct_reg = 0;
		YX_JTU1_LinkReactivate(DELAY_REG);
		if (YX_JT_LinkCanCom()) {                                              /* 其他链路正常,需要更换参数 */
		    s_lcb.ipflag = FALSE;                                              /* 域名解析的IP无效 */
            s_lcb.curindex++;
            CheckGprsPara();
        ...
        OS_PostMsg(PRIO_OPTTASK, MSG_UDPLINK_REGERR, COM_UDP, 0);
    }
}
...
static void Turninto_Loging(void)
{
    if (++s_lcb.ct_log <= MAX_LOG) {
        #if DEBUG_ULINK > 0
        printf_com("<第一服务器UDP登入请求(%d)>\r\n", s_lcb.ct_log);
        #endif
        
        YX_StartTmr(s_monitortmr, PERIOD_LOG);
        
        DAL_UDP_ClearBuf(COM_UDP);
        YX_JTU1_SendReqLogin();                                                /* 发送登入帧 */
    } else {
        #if DEBUG_ULINK > 0
        printf_com("<第一服务器UDP登入请求超过次数>\r\n");
        #endif
        
        s_lcb.ct_log = 0;
        YX_JTU1_LinkReactivate(0);                                             /* 重新连接 */
...
{
    DAL_PP_ClearParaByID(PP_ID_AUTHCODE1);                                           /* 清除鉴权码 */
...
static void DiagnoseProc(void)
{
    if (s_lcb.ct_user > 0) {                                                   /* 有用户，必须处于激活 */
        OS_ASSERT(YX_JTU1_LinkIsActived(), RETURN_VOID);
    }
    
    if (YX_JTU1_LinkIsActived()) {                                             /* 处于激活状态 */
...
{
    #if DEBUG_ULINK > 0
    printf_com("<YX_JTU1_LinkInformGprsActived(%x)>\r\n", result);
    #endif
    ...
    if ((s_lcb.status & (ACTIVE_ | MASK_)) == (ACTIVE_ | CONNECT_)) {          /* 已激活应用 */
        if (result == 0) {                                                     /* 注册成功 */
            ...
            if (slen > 0 && slen < sizeof(authinfo.authcode)) {
                ...
                DAL_PP_StoreParaByID(PP_ID_AUTHCODE1, (INT8U *)&authinfo, sizeof(authinfo));
    ...
    if ((s_lcb.status & (ACTIVE_ | MASK_)) == (ACTIVE_ | CONNECT_ | REGED_)) { /* 已激活应用 */
        if (result) {
            ...
            if (++s_lcb.ct_logfail >= MAX_LOG) {                               /* 登入失败应答超过次数 */
                ...
                OS_PostMsg(PRIO_OPTTASK, MSG_UDPLINK_LOGERR, COM_UDP, 0);    
            }
        }
    }
}
...
{
    if (DAL_GPRS_ComIsSuspended() || !YX_JTU1_LinkIsLoged()) {
        return FALSE;
    } else {
        return TRUE;
    }
...
{
    #if DEBUG_ULINK > 0
    printf_com("<YX_JTU1_LinkApply(%d)>\r\n", s_lcb.ct_user + 1);
    #endif
...
{
    #if DEBUG_ULINK > 0
    printf_com("<YX_JTU1_LinkRelease,delay(%d),user(%d)>\r\n", delay, s_lcb.ct_user - 1);
    #endif
...
{
    #if DEBUG_ULINK > 0
    printf_com("<YX_JTU1_LinkReactivate,delay(%d),user(%d)>\r\n", delay, s_lcb.ct_user);
    #endif
