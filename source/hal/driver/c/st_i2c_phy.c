...
** 返回:     无
********************************************************************/
static void PinsConfig(const I2C_REG_T *pinfo)
{
    ...
    GPIO_PinAFConfig((GPIO_TypeDef *)pinfo->gpio_base, pinfo->pin_scl, GPIO_AF_1);
    ...
    GPIO_Init((GPIO_TypeDef *)pinfo->gpio_base, &gpio_initstruct);
    GPIO_PinAFConfig((GPIO_TypeDef *)pinfo->gpio_base, pinfo->pin_sda, GPIO_AF_1);
}
...
** 返回:     无
********************************************************************/
static void I2C_Config(const I2C_REG_T *pinfo)
{
	I2C_InitTypeDef I2C_InitStruct;
	
	I2C_InitStruct.I2C_Mode                = I2C_Mode_I2C;                     /* 配置模式 */
	I2C_InitStruct.I2C_Ack                 = I2C_Ack_Enable;                   /* 应答使能 */
	I2C_InitStruct.I2C_AcknowledgedAddress = I2C_AcknowledgedAddress_7bit;     /* 地址位数 */
	I2C_InitStruct.I2C_OwnAddress1         = 0x00;                             /* 本机地址 */
	I2C_InitStruct.I2C_AnalogFilter        = I2C_AnalogFilter_Enable;          /* 使能模拟噪声滤波 */
    I2C_InitStruct.I2C_DigitalFilter       = 0x00;                             /* 数字噪声滤波 */
    I2C_InitStruct.I2C_Timing              = 0x00210507;                       /* 设置时钟寄存器的值 */
    
	I2C_Init(pinfo->i2c_base, &I2C_InitStruct);                                /* 初始化I2C总线 */
	I2C_Cmd(pinfo->i2c_base, ENABLE);                                          /* 使能I2C总线 */
}
...
{
    I2C_GenerateSTART(pinfo->i2c_base, ENABLE);
...
BOOLEAN ST_I2C_OpenCom(INT8U com)
{
    ...
    PinsConfig(pinfo);                                                         /* configure the rx & tx pins */
