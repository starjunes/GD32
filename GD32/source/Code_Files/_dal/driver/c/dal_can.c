...
typedef struct {
    CAN_CBINDEX_E index;                                             /* 回调索引号 */
    void  (*lbhandle) (INT8U chn);                                   /* 回调函数指针 */
} CAN_CBAPP_E;
...
/*****************************************************************************
**  函数名:  CanBusOffTestSendPack
**  函数描述: 用于促发bus off，且因为网管需要在bus off后发limphome报文，所以用
              limphome报文做促发
**  参数:    [in] com :
**  返回:    无
*****************************************************************************/
static void CanBusOffTestSendPack(INT8U chn)
{
    ...
    can_interrupt_disable(CANx, CAN_INT_TME); /* 关闭发送中断 */ 
    can_transmission_stop(CANx, 0); 
    can_interrupt_enable(CANx, CAN_INT_TME);  /* 打开发送中断 */
