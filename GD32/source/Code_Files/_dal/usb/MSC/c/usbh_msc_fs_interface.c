...
int8_t if_readBuf(hwInterface* file,uint32_t address,void* buf)
{
  ...
  {  
    do
    {
      status = USBH_MSC_Read10(&USB_OTG_Core, buf, address, 512);
...
int8_t if_writeBuf(hwInterface* file,uint32_t address,void* buf)
{
  int8_t status = EFS_ERROR;
  int32_t waittime;
  ...
  {  
    do
    {
      status = USBH_MSC_Write10(&USB_OTG_Core, buf, address, USBH_MSC_PAGE_LENGTH);
      USBH_MSC_HandleBOTXfer(&USB_OTG_Core, &USB_Host);
    }
...
int8_t if_TestUnitReady(void)
{
  ...
  {  
    do
    {
      status = USBH_MSC_TestUnitReady(&USB_OTG_Core);
      USBH_MSC_HandleBOTXfer(&USB_OTG_Core, &USB_Host);
    }
    while((status == USBH_MSC_BUSY ) && \
      (HCD_IsDeviceConnected(&USB_OTG_Core)));
...
int8_t if_RequestSense(void)
{
  ...
  {  
    ...
    do
    {
      status = USBH_MSC_RequestSense(&USB_OTG_Core);
      USBH_MSC_HandleBOTXfer(&USB_OTG_Core, &USB_Host);
    }
    while((status == USBH_MSC_BUSY ) && \
      (HCD_IsDeviceConnected(&USB_OTG_Core)));
