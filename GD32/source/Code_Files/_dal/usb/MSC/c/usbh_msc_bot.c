...
void USBH_MSC_HandleBOTXfer (USB_OTG_CORE_HANDLE *pdev ,USBH_HOST *phost)
{
  ...
  {  
    ...
    #if DEBUG_DATATEST1 > 0
      Debug_SysPrint("SEND_CBW %d: ", USBH_MSC_BOT_CBW_PACKET_LENGTH);
      Debug_PrintHex(&USBH_MSC_CBWData.CBWArray[0], USBH_MSC_BOT_CBW_PACKET_LENGTH);
    #endif
    ...
    case USBH_MSC_BOT_DATAIN_STATE:
      ...
      if((URB_Status == URB_DONE) ||(USBH_MSC_BOTXferParam.BOTStateBkp != USBH_MSC_BOT_DATAIN_STATE))
      {
        ...
        #if DEBUG_DATATEST1 > 0
          Debug_SysPrint("BOT_DATAIN_STATE %d: ", MSC_Machine.MSBulkInEpSize);
          Debug_PrintHex(datapointer, MSC_Machine.MSBulkInEpSize);
        #endif
        ...
        {       
          USBH_BulkReceiveData (pdev,
			        ...
			        MSC_Machine.hc_num_in);
        #if DEBUG_DATATEST1 > 0
          Debug_SysPrint("BOT_DATAIN_STATE %d: ", remainingDataLength);
          Debug_PrintHex(datapointer, remainingDataLength);
        #endif
    ...
    case USBH_MSC_BOT_DATAOUT_STATE:
      ...
      if(URB_Status == URB_DONE)
      {
        ...
        if(remainingDataLength > MSC_Machine.MSBulkOutEpSize)
        {
          USBH_BulkSendData (pdev,
                             ...
                             MSC_Machine.hc_num_out);
        #if DEBUG_DATATEST1 > 0
          Debug_SysPrint("BOT_DATAOUT_STATE Send %d: ", MSC_Machine.MSBulkOutEpSize);
          Debug_PrintHex(datapointer, MSC_Machine.MSBulkOutEpSize);
        #endif
          datapointer_prev = datapointer;
        ...
        else
        {
          USBH_BulkSendData (pdev,
			     ...
			     MSC_Machine.hc_num_out);
        #if DEBUG_DATATEST1 > 0
          Debug_SysPrint("BOT_DATAOUT_STATE Send %d: ", remainingDataLength);
          Debug_PrintHex(datapointer, remainingDataLength);
        #endif
          remainingDataLength = 0; /* Reset this value and keep in same state */   
        }      
      ...
      else if(URB_Status == URB_NOTREADY)
      {
        ...
        #if DEBUG_DATATEST1 > 0
          Debug_SysPrint("BOT_DATAOUT_STATE Send %d: ", MSC_Machine.MSBulkOutEpSize);
          Debug_PrintHex(datapointer - MSC_Machine.MSBulkOutEpSize, MSC_Machine.MSBulkOutEpSize);
        #endif
        }
        else
        {
    ...
    case USBH_MSC_RECEIVE_CSW_STATE:
      /* BOT CSW stage */     
        ...
        USBH_BulkReceiveData (pdev,
                              ...
                              MSC_Machine.hc_num_in);
      #if DEBUG_DATATEST1 > 0
        Debug_SysPrint("RECEIVE_CSW_STATE %d: ", USBH_MSC_CSW_MAX_LENGTH);
        Debug_PrintHex(USBH_MSC_BOTXferParam.pRxTxBuff, USBH_MSC_CSW_MAX_LENGTH);
      #endif
