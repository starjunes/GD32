...
USBH_Status USBH_CtlSendSetup ( USB_OTG_CORE_HANDLE *pdev, 
  ...
  pdev->host.hc[hc_num].xfer_len = USBH_SETUP_PKT_SIZE;   
        ...
        Debug_SysPrint("USBH_CtlSendSetup %d: ", hc_num);
        Debug_PrintHex(buff, hc_num);
      #endif
                                ...
                                uint8_t hc_num)
{
 ...
 { /* Put the PID 1 */
  ...
  HCD_SubmitRequest (pdev , hc_num);   
   #if DEBUG_DATATEST1 > 0
        Debug_SysPrint("USBH_CtlSendData %d: ", length);
        Debug_PrintHex(buff, length);
      #endif
  return USBH_OK;
}
                                ...
                                uint8_t hc_num)
{
  ...
  HCD_SubmitRequest (pdev , hc_num);   
  #if DEBUG_DATATEST1 > 0
        Debug_SysPrint("USBH_CtlReceiveData %d: ", length);
        Debug_PrintHex(buff, length);
      #endif
  return USBH_OK;
...
#if DEBUG_DATATEST1 > 0
        Debug_SysPrint("USBH_InterruptReceiveData %d: ", length);
        Debug_PrintHex(buff, length);
      #endif
...
{
#if DEBUG_DATATEST1 > 0
        Debug_SysPrint("USBH_InterruptSendData %d: ", length);
        Debug_PrintHex(buff, length);
      #endif
  pdev->host.hc[hc_num].ep_is_in = 0;  
  pdev->host.hc[hc_num].xfer_buff = buff;
...
{    
  ...
  HCD_SubmitRequest (pdev , hc_num);  
  #if DEBUG_DATATEST1 > 0
        Debug_SysPrint("USBH_IsocReceiveData %d: ", length);
        Debug_PrintHex(buff, length);
      #endif
  return USBH_OK;
}
                                ...
                                uint8_t hc_num)
{
  ...
  HCD_SubmitRequest (pdev , hc_num);  
  #if DEBUG_DATATEST1 > 0
        Debug_SysPrint("USBH_IsocSendData %d: ", length);
        Debug_PrintHex(buff, length);
      #endif
  return USBH_OK;
}
